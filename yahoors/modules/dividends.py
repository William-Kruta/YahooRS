...
class Dividends:
    ...
    def _read_dividends(self, tickers: list[str]) -> pl.DataFrame:
        if isinstance(tickers, str): 
            tickers = [tickers]
        tickers = clean_tickers(tickers)
        df = self.conn.execute(f"SELECT * FROM {self.table_name} WHERE ticker IN ({', '.join(['?'] * len(tickers))}) ORDER BY date ASC, ticker", tickers).pl()
        return df

    def _insert_dividends(self, df: pl.DataFrame):
        if df.is_empty():
            return
        insert_data(df, ["date", "ticker", "dividend", "ttm_dividend", "close", "dividend_yield_pct"], self.table_name, self.conn, pk_cols=["date", "ticker"])
    ...
    def _read_dividend_info(self, tickers: list[str]) -> pl.DataFrame:
        if isinstance(tickers, str): 
            tickers = [tickers]
        tickers = clean_tickers(tickers)
        df = self.conn.execute(f"SELECT * FROM dividend_info WHERE ticker IN ({', '.join(['?'] * len(tickers))})", tickers).pl()
        return df
...
def calculate_dividend_yield(
    ...
    result = (
        ...
        .sort("date")
        .join_asof(
            candles.select(["date", "ticker", "close"]).sort("date"),
            on="date",
            by="ticker",
            strategy="backward",
        )
