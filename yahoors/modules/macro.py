...
class Macro:
    ...
    def get_yield_curve(
        ...
        short_term_df = self.get_risk_free_rate(
            ticker=short_term_ticker, interval=interval, period=period
        )
        long_term_df = self.get_risk_free_rate(
            ticker=long_term_ticker, interval=interval, period=period
        )
        df = (
            short_term_df.select(["date", "close"])
    ...
    def get_currency_exchange_rate(
        self, currency_a: str, currency_b: str, interval: str = "1d", period: str = "max"
    ):
        df = self.candles.get_candles(
            tickers=f"{currency_a}{currency_b}=X", interval=interval, period=period
        )
        return df
